...
vpnip=${vpnip} private_key=${private_key} port=${port} envsubst < ${servertemplate} >> ${output}/${server}/${server}.conf
...
while read line; do
  ...
  vpnip=${vpnip} peer=${i} public_key=${public_key} preshared_key=${preshared_key} localnet=${localnet} \
    envsubst < ${peersectiontemplate} >> ${output}/${server}/${server}.conf
  vpnip=${vpnip} peer=${i} server_public_key=${server_public_key} preshared_key=${preshared_key} localnet=${localnet} private_key=${private_key}\
